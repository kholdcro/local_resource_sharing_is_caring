...
class MqttHost(threading.Thread):
    ...
    def publishGlobal(self, message, qos=0):
        self.client.publish("esp/rec", message, qos=qos)
    ...
    def publishLocal(self, msg, addr, qos=0):
        ...
        self.client.publish("{}/r".format(addr), msg, qos=qos)
        return
        if qos != 0:
            self.client.publish("{}/1r".format(addr), msg, qos=qos)
        else:
            self.client.publish("{}/r".format(addr), msg, qos=qos)
