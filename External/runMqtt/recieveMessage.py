...
def recieveExternalInput(msg, wifi_host):
   ...
   try:
      ...
      pyld = bytearray(str.encode(msg.payload[0:4].decode('UTF-8')))
      pyld.extend(msg.payload[5:])

   
   if publishTopic == 'l':
      wifi_host.publishLocal(pyld, espNum, qos=msg.qos)
   elif publishTopic == 'g':
      wifi_host.publishGlobal(pyld, qos=msg.qos)
   else:
