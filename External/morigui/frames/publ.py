...
class PublishLocal():
    ...
    def publishBinary(self):
        ...
        message = bytearray(str.encode("vel"))
        # message = bytearray(str.encode("kns"))
        # print("WTF")

        text = text.rsplit(' ')
        print(text)
        # print(struct.pack("f",float(text[0])))
        for txt in text:
            message.extend(str.encode(" "))
            if txt != '-':
                message.extend(struct.pack("f",float(txt)))
            else: 
                message.extend(str.encode("-"))
        # for txt in text:
        #     if txt.isdigit():
