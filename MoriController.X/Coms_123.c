...
int16_t NbrValInt[3] = {0, 0, 0}; // integral value received by neighbour
...
int16_t NbrCmdVel[3] = {0, 0, 0}; // velocity command recieved by neighbour
...
//    static uint8_t EdgInIdleBytes[3] = {0, 0, 0};
    ...
    while(bytes_read < 64) { // ActHandle sends max 11 bytes at 20Hz (allow 3)
        bytes_read++;
        ...
        if (EdgInCase[edge] != 40) {//Only read alloc byte if in relay
            ...
            if ((bytes_read > 64)) {
                //If in relay, then it's number of cycles.
                //bytes_read deceiving, if in relay, then number of times RX or TX is not ready.
                //Could repeat quickly
        ...
        switch (EdgInCase[edge]) { // select case set by previous byte
            ...
            case 7:
                if (EdgInAloc[edge] & 0b00001000) { // angle command
                    if (NbrCmdIDn[edge] == VEL_CMDID)
                        NbrCmdVel[edge] = 0xFF00 & (((int16_t)EdgIn) << 8);
                    else
                        NbrCmdAng[edge] = 0xFF00 & (((int16_t)EdgIn) << 8);
                    EdgInCase[edge] = 8;
                    break;
                }
            case 8:
                if (EdgInAloc[edge] & 0b00001000) {
                    if (NbrCmdIDn[edge] == VEL_CMDID)
                        NbrCmdVel[edge] = NbrCmdVel[edge] | (int16_t) EdgIn;
                    else
                        NbrCmdAng[edge] = NbrCmdAng[edge] | (int16_t) EdgIn;
                    EdgInCase[edge] = 9;
                    break;
                }
            case 9:
            ...
            case 10:
                if (EdgInAloc[edge] & 0b00001000) {
                    NbrValInt[edge] = 0xFF00 & (((int16_t)EdgIn) << 8);
                    EdgInCase[edge] = 11;
                    break;
                }
            case 11:
                if (EdgInAloc[edge] & 0b00001000) {
                    NbrValInt[edge] = NbrValInt[edge] | (int16_t) EdgIn;
                    EdgInCase[edge] = 12;
                    break;
                }
            ...
            case 12: // verify action command
                if (EdgIn == EDG_End) {
                    ...
                    if (Flg_SensFus)
                        Sens_MOD_SetLiveOffset(edge, NbrValAng[edge]);
                    else
...
void Coms_123_ActHandle() { // called in tmr3 at 20Hz
    ...
    for (edge = 0; edge < 3; edge++) {
        if (Flg_EdgeSyn[edge]) {
            ...
            if ((Flg_EdgeReq_Ang[edge] && FLG_MotRot_Active && MODE_MotRot_Active) || Flg_VelCtrl)
                byte = byte | 0b00001000;
            if (Flg_EdgeReq_Cpl[edge])// && MODE_Cplngs_Active)
                byte = byte | 0b00000100;
            if (NbrCmdMatch[edge] || Flg_VelCtrl) { // neighbour matched own cmd
                if (FLG_WaitAllEdges && (!Flg_VelCtrl)) {
            ...
            Coms_123_Write(edge, byte);
            if(Flg_VelCtrl){
                Coms_123_Write(edge, VEL_CMDID);
//                    LED_R = LED_On;
            } else {
//                    LED_R = LED_Off;
                Coms_123_Write(edge, CMD_ID);
            }                
            // send angle reading for averaging
            uint16_t selfAngle = Acts_ROT_GetAngle(edge, false);
            if (Flg_SensFus)
                selfAngle = Sens_MOD_GetAngleFormatted(edge, false);
            ...
            if (Flg_VelCtrl) {
                int16_t VelTemp = (int16_t) Acts_ROT_SendVelTarget(edge);
                Coms_123_Write(edge, (uint8_t) ((VelTemp & 0xFF00) >> 8));
                Coms_123_Write(edge, (uint8_t) (VelTemp & 0x00FF));
            } 
            else if (Flg_EdgeReq_Ang[edge]) {
            ...
            if (Flg_VelCtrl || Flg_EdgeReq_Ang[edge]){ 
...
void Coms_123_ActVerify(uint8_t edge) {
    ...
    if (NbrCmdIDn[edge] != CMD_ID){
        ...
        // Either angle or velocity control
        if (EdgInAloc[edge] & 0b00001000){
            if (NbrCmdIDn[edge] == VEL_CMDID) {
                Acts_ROT_SetVelNeighbourTarget(edge, NbrCmdVel[edge]);
                Acts_ROT_SetSPDAvgNeighbour(edge, NbrValInt[edge]);
            }
            // angle command verification
            else if (Flg_EdgeReq_Ang[edge]) {         
                if (NbrValSpd[edge] != Acts_ROT_GetSpeedLimit(edge))
                    NbrCmdNoGo = true; // values do not match, NOGO   
                else if (NbrCmdAng[edge] != Acts_ROT_GetTarget(edge))
                    NbrCmdNoGo = true; // values do not match, NOGO
...
void Coms_123_Send_Accel_Values() {
    for (uint8_t edge = 0; edge < 3; edge++) {
        if (!Flg_EdgeSyn[edge]) continue;
        //Header = 0b11000000 | 0b00000011 
        //Header = Coms_CMD | Interpret neighbour accel 
        Coms_123_Write(edge, 0b11000011); //Coms_CMD 3
        
        const struct ground_state loc = Sens_GRD_get_ground_state_edge_frame(edge);

        uint8_t c_Index;
        uint8_t *f_Data = ( uint8_t* ) &loc.ground.x;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; Coms_123_Write(edge, f_Data[ --c_Index ]) ); 

        f_Data = ( uint8_t* ) &loc.ground.y;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; Coms_123_Write(edge, f_Data[ --c_Index ]) ); 

        f_Data = ( uint8_t* ) &loc.ground.z;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; Coms_123_Write(edge, f_Data[ --c_Index ]) ); 

        f_Data = ( uint8_t* ) &loc.velocity.x;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; Coms_123_Write(edge, f_Data[ --c_Index ]) ); 

        f_Data = ( uint8_t* ) &loc.velocity.y;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; Coms_123_Write(edge, f_Data[ --c_Index ]) ); 

        f_Data = ( uint8_t* ) &loc.velocity.z;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; Coms_123_Write(edge, f_Data[ --c_Index ]) );         
...
void Coms_123_Send_Local_Angle() {
    if(FLG_Disable_Hall_sensor) return;
    
    for (uint8_t edge = 0; edge < 3; edge++) {
        if (!Flg_EdgeSyn[edge]) continue;

//        const float angle = Sens_ENC_GetKalmanPos(edge, false);
        const float angle = Sens_ENC_Get(edge, false)+180.f;
        
        Coms_123_Write(edge, 0b11000101); //Coms_CMD 5
