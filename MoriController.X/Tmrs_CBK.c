...
void Tmrs_CBK_Timer1_Handle(void) {
//    Coms_123_ActHandle(); // action synchronisation handle
    //Note - could possibly put Sens_FUS_execute() here and just put counters in the interrupt
    Sens_MOD_iterate_filter();    
//    Coms_ESP_Verbose_Float(Sens_EDG_get_angle_est(1));
//    Coms_ESP_Verbose_One_Byte(6);
//    Coms_ESP_Verbose_Float(Sens_ENC_Get(1, false));
//    Coms_ESP_Verbose_Float(Sens_ENC_GetKalmanPos(1, false));
...
//    if (MODE_ACC_CON) Flg_i2c_ACC = true; // read accelerometer, called in tmr1
    ...
    Sens_MOD_update_connections(Flg_EdgeSyn);
    ...
    for (edge = 0; edge < 3; edge++) { // extension control loops
     ...
     Coms_123_Send_Accel_Values(); // sends accelerometer values, inedge = N/A(100), accum = 0, dist = 0
...
void Tmrs_CBK_UpdateStableFlag(bool *flg_stable_state) {
    ...
    for (edge = 0; edge < 3; edge++){
        ...
        if (Flg_EdgeReq_Ang[edge]) {
            bool inrange;
            if (!Flg_SensFus)
                inrange = Acts_ROT_InRange(edge, Acts_ROT_GetAngle(edge, true));
            else
                inrange = Acts_ROT_InRange(edge, Sens_MOD_GetAngleFormatted(edge, true));
            if (!inrange)
                out = false;            
        }
    }
