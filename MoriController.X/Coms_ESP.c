...
uint8_t ESP_Update_Delay_Velocity = 100;
uint8_t ESP_Update_Delay_AngleFlt = 100;
...
bool Coms_ESP_Handle(uint8_t byte) {
    ...
    switch (state) {
        ...
        case 3:
            ...
            break;     
//        case 4:
//            if (Coms_ESP_Set_Sens_FUS_dist(byte))
//                return Coms_CMD_Reset(&state, &alloc); 
//            break;
...
void Coms_ESP_Verbose_Float(float data) {
    UART4_Write(0b01100000);
    uint8_t *f_Data = ( uint8_t* ) &data;
    uint8_t c_Index;
    for(c_Index = 0; c_Index < sizeof( float ) ; UART4_Write(f_Data[ c_Index++ ]) );                            
    UART4_Write(ESP_End);
}
...
//    UART4_Write((uint8_t)FLG_Disable_Hall_sensor);
//    UART4_Write((uint8_t)FLG_Disable_ACC_sensor);
////    UART4_Write((uint8_t)FLG_Disable_Gnd_sensor);
//    UART4_Write((uint8_t)Flg_Tmr1); // to go
}

void Coms_ESP_Request_Angles() {
    UART4_Write(0b10010101);
    // Coms_ESP_Write_AngleFlt();
    for (uint8_t i = 0; i < 3; i++) {
        float angle;
        angle =((float)Sens_ENC_GetKalmanPos(i, false));
        uint8_t *f_Data = ( uint8_t* ) &angle;
        uint8_t c_Index;
        for(c_Index = 0; c_Index < sizeof( float ) ; UART4_Write(f_Data[ c_Index++ ]) );                            
    }    
    UART4_Write(ESP_End);
...
//    }
    ...
    for (i = 0; i < 3; i++) {
        float delta;
//        if (Flg_VelCtrl) delta = (float)(Sens_ENC_GetKalmanVel(i)) * MotRot_PID_freq;
//        delta = Acts_ROT_GetSPDAvgOut(i);
//        delta = (float)Acts_ROT_GetTarget(i);
        delta = Acts_ROT_GetUnsaniTarget(i);
        uint8_t *f_Data = ( uint8_t* ) &delta;
        uint8_t c_Index;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; UART4_Write(f_Data[ --c_Index ]) );                            
    }        
}


void Coms_ESP_Write_Sens_Fus() {
    for (uint8_t i = 0; i < 3; i++) {
        float angle = Sens_EDG_get_angle_est(i);
        uint8_t *f_Data = ( uint8_t* ) &angle;
        uint8_t c_Index;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; UART4_Write(f_Data[ --c_Index ]) );                            
    }           
...
void Coms_ESP_Write_Orient() {
    uint8_t i;
    for (i = 0; i < 3; i++) {
        // UART4_Write16(Sens_ACC_GetRaw(i));
//        UART4_Write16(Acts_ROT_GetSPDAvgNeighbour(i));
       UART4_Write16(Sens_ACC_GetAngle(i));
    }
}


void Coms_ESP_Write_Velocity() {
//    struct ground_state ground_filter = Sens_GRD_get_ground_estimate();
//    float tmp[3];
//    tmp[0] = ground_filter.ground.x;
//    tmp[1] = ground_filter.ground.y;
//    tmp[2] = ground_filter.ground.z;
    ...
    for (i = 0; i < 3; i++) {
        float delta;
//        delta = (float)(Sens_ENC_GetDelta(i)) * MotRot_PID_freq;        
        // delta = (float)(Sens_ENC_GetKalmanVel(i)) * MotRot_PID_freq;        
//        delta = (float)(Sens_FUS_get_weight(2,i));        
//        delta = tmp[i];
        delta = Sens_MOD_get_ground_angle(i);
        uint8_t *f_Data = ( uint8_t* ) &delta;
        uint8_t c_Index;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; UART4_Write(f_Data[ --c_Index ]) );                            
    }    
...
void Coms_ESP_Write_AngleFlt() {
    uint8_t i;
    
    for (i = 0; i < 3; i++) {
        float angle;
//        if (Flg_VelCtrl) angle = (float)(Sens_ENC_GetKalmanPos(i, false)) + 180.0f;
//        angle =((float)Sens_ENC_Get(i, false)) + 180.0f;
//        angle =((float)Sens_ENC_GetKalmanPos(i, false)) + 180.0f;
//        angle =((float)Sens_ENC_GetKalmanPos(i, false));
        angle =((float)Sens_ENC_Get(i, false)+180.f);
        uint8_t *f_Data = ( uint8_t* ) &angle;
        uint8_t c_Index;
        for(c_Index = sizeof( float ) ; 0 < c_Index ; UART4_Write(f_Data[ --c_Index ]) );                            
    }    
...
void Coms_ESP_StateUpdate(void) {
    ...
    // To do: Combine into simple array (because why not)
    static uint8_t ESP_DataLog_Time_Angle;
    static uint8_t ESP_DataLog_Time_Edge;
    static uint8_t ESP_DataLog_Time_Orient;
    static uint8_t ESP_DataLog_Time_Velocity;
    static uint8_t ESP_DataLog_Time_AngleFlt;
    ...
    if (Coms_ESP_Data_Transmission & 0b00001000)
        ESP_DataLog_Time_Velocity += ESP_DataLog_Time_Elapsed;
    if (Coms_ESP_Data_Transmission & 0b00010000)
        ESP_DataLog_Time_AngleFlt += ESP_DataLog_Time_Elapsed;
    ...
    if (ESP_DataLog_Time_Velocity >= ESP_Update_Delay_Velocity) {
        ESP_Update_Reset_Trigger |= 0b00001000;
        ESP_DataLog_Time_Velocity %= ESP_Update_Delay_Velocity;
    }    

    if (ESP_DataLog_Time_AngleFlt >= ESP_Update_Delay_AngleFlt) {
        ESP_Update_Reset_Trigger |= 0b00010000;
        ESP_DataLog_Time_AngleFlt %= ESP_Update_Delay_AngleFlt;
    }    
    
    
    // If no fields need update
    if ((ESP_Update_Reset_Trigger & 0b00011111) == 0) return;
    
    UART4_Write((0b11000000 | (ESP_Update_Reset_Trigger & 0b00011111))); //New alloc byte
    
    if (ESP_Update_Reset_Trigger & 0b00000001) 
        Coms_ESP_Write_Sens_Fus();
    ...
    if (ESP_Update_Reset_Trigger & 0b00001000)
        Coms_ESP_Write_Velocity();
    
    if (ESP_Update_Reset_Trigger & 0b00010000)
        Coms_ESP_Write_AngleFlt();      
...
bool Coms_ESP_SetDatalogPeriod(uint8_t byte) {
    ...
    if (count >= 2) {
        if (byte == ESP_End) {
            ...
            else if (requested_data & 0b00001000) {
                ESP_Update_Delay_Velocity = period;
            }
            else if (requested_data & 0b00010000) {
                ESP_Update_Delay_AngleFlt = period;
            }
            else {
                ESP_Update_Delay_Angle = period;
                ESP_Update_Delay_Edge = period;
...
bool Coms_ESP_Set_Sens_FUS_dist(uint8_t byte) {
//    static uint8_t count = 0;
//    static uint8_t dist;
//    if (count >= 1) {
//        if (byte == ESP_End) {
//            Coms_123_Set_Sens_FUS_relay_distance(dist);
//        } else {
//            Coms_ESP_OverflowError();
//        }
//        count = 0;
