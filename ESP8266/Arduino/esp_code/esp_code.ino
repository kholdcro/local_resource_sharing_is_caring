...
const char* brn_ssid PROGMEM = "rrl_wifi";
const char* brn_password PROGMEM =  "Bm41254126";
...
const char* mqttServer PROGMEM = "192.168.1.2";
...
const char * const cmdLine[] PROGMEM = {"mac", "gver", "bver", "spudp", "hello",
                ...
                "f1f", "f2t", "f2f", "f3t", "f3f", 
                "envel", "stvel", "f5t", "f5f", "wiggles",
                "drcoup", "dlflag", "dlperiod", "cops", "name",
                "spd", "stp", "torque", "rer", "dbd",
                "pid", "ksp", "rok", "velocity", "kns", // ksp == kerbel space program == gain for speed control
                "whl", "idle", "dist"};  
int numCmds;
...
unsigned long last_vel_cmd = millis();
...
void startMQTT()
{
  ...
  client.subscribe(recieveName, 0); //1 is qos level
  client.subscribe("esp/rec");
  char recieveNameQosOne[36];
  sprintf(recieveNameQosOne, "%s/1r", clientName);
  client.subscribe(recieveNameQosOne, 1); //1 is qos level
}
